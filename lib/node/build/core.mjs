...
function fingerprintAssets(stagingSiteDir, statusMap) {
  ...
  for (let round = 0; round < FINGERPRINT_MAX_ROUNDS; round++) {
    ...
    const mappings = Array.from(targetUrls.entries())
      .map(([rel, url]) => [`/${rel}`, url])
    ...
    stable = Array.from(targetUrls.entries()).every(
      ([rel, url]) => hashName(rel, readFileSync(join(stagingSiteDir, rel))) === url
    );
    if (stable) break;
  }
  ...
  for (const [rel, url] of targetUrls) {
    const newRel = url.slice(1);
