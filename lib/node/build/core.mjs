...
const MINIFY_ASSET_EXTENSIONS = new Set([".css", ".js"]);
...
function stageAssetGroup(sourceRoot, sourceFiles, stagingSiteDir, targetPrefix, statusMap, label) {
  ...
  for (const sourcePath of sourceFiles) {
    ...
    const shouldMinify = targetPrefix === "assets" && MINIFY_ASSET_EXTENSIONS.has(extension);
    ...
    if (shouldMinify) {
      ...
      let minifiedCode;
      try {
        minifiedCode = transformSync(sourceCode, {
          loader,
          minify: true,
          legalComments: "none",
          charset: "utf8",
        ...
        throw new Error(`Failed to minify ${outputRel}: ${message}`);
      }

      ensureDirForFile(stagingPath);
      writeFileSync(stagingPath, minifiedCode, "utf8");
...
const FINGERPRINT_HASH_LENGTH = 10;
const FINGERPRINT_REWRITE_EXTENSIONS = new Set([".html", ".css", ".js"]);
const FINGERPRINT_MAX_ROUNDS = 6;

// giscus 运行在跨站 iframe 中，父页面预取受浏览器缓存分区限制无法被其复用，
// 因此将四个主题样式表内联为 data URI，主题切换无需任何网络请求。
const INLINE_ASSET_RELS = new Set([
  "assets/giscus-white.css",
  "assets/giscus-gray-10.css",
  "assets/giscus-gray-90.css",
  "assets/giscus-gray-100.css",
]);

const inlineCssDataUri = (content) =>
  `data:text/css;base64,${Buffer.from(content, "utf8").toString("base64")}`;

// 为 staging/assets 下的资源生成内容哈希文件名，并重写站点内全部引用。
function fingerprintAssets(stagingSiteDir, statusMap) {
  const stagingAssetsDir = join(stagingSiteDir, "assets");
  const assetFiles = walkFiles(stagingAssetsDir);
  if (assetFiles.length === 0) return;

  const hashName = (rel, content) => {
  ...
  // CSS 中的相对 url() 引用需先归一化为绝对 /assets/ 路径，才能被字符串替换命中。
  const normalizeCssUrls = (content, fileRelPath) => {
    const dir = posix.dirname(fileRelPath);
  ...
  // 所有可能引用 assets 的文本文件，保留原始内容以便每轮从头重写。
  const rewriteFiles = walkFiles(stagingSiteDir).filter((file) =>
    FINGERPRINT_REWRITE_EXTENSIONS.has(extname(file).toLowerCase())
  );
  const originals = new Map(
    rewriteFiles.map((file) => {
      const rel = normalizePosixPath(relative(stagingSiteDir, file));
      const raw = readFileSync(file, "utf8");
      const content = extname(file).toLowerCase() === ".css"
        ? normalizeCssUrls(raw, rel)
        : raw;
      return [file, content];
    })
  );

  // 资源相对路径 -> 目标 URL；重写会改变部分资源内容，迭代直至哈希稳定。
  const targetUrls = new Map(
    assetFiles.map((absPath) => [
      normalizePosixPath(relative(stagingSiteDir, absPath)),
      undefined,
    ])
  );

  let stable = false;
  for (let round = 0; round < FINGERPRINT_MAX_ROUNDS; round++) {
    for (const rel of targetUrls.keys()) {
      if (!INLINE_ASSET_RELS.has(rel)) {
        targetUrls.set(rel, hashName(rel, readFileSync(join(stagingSiteDir, rel))));
      }
    }

    const mappings = Array.from(targetUrls.keys()).map((rel) => {
      const content = readFileSync(join(stagingSiteDir, rel));
      const to = INLINE_ASSET_RELS.has(rel)
        ? inlineCssDataUri(content)
        : targetUrls.get(rel);
      return [`/${rel}`, to];
    });
    ...
    stable = Array.from(targetUrls.keys()).every((rel) => {
      const content = readFileSync(join(stagingSiteDir, rel));
      if (INLINE_ASSET_RELS.has(rel)) {
        return inlineCssDataUri(content) === mappings.find(([from]) => from === `/${rel}`)[1];
      }
      return hashName(rel, content) === targetUrls.get(rel);
    });
    if (stable) break;
  }

  if (!stable) {
    throw new Error("Asset fingerprinting failed to converge; check cross-asset references.");
  }

  for (const rel of targetUrls.keys()) {
    if (INLINE_ASSET_RELS.has(rel)) {
      rmSync(join(stagingSiteDir, rel), { force: true });
      statusMap.delete(rel);
      continue;
    }

    const newRel = targetUrls.get(rel).slice(1);
    if (newRel === rel) continue;

    const newPath = join(stagingSiteDir, newRel);
    ensureDirForFile(newPath);
    renameSync(join(stagingSiteDir, rel), newPath);

    const status = statusMap.get(rel);
    statusMap.delete(rel);
    if (status) statusMap.set(newRel, status);
  }
}

// 生成 Cloudflare Pages 的 _headers：带哈希的 assets 永久缓存，其余每次协商。
function writeHeadersFile(stagingSiteDir, statusMap) {
  const content = [
    "/assets/*",
  ...
  writeFileSync(join(stagingSiteDir, "_headers"), content, "utf8");
  upsertStatus(statusMap, "_headers", "updated");
}
...
export async function buildSite({ outDirName, jobs, configPath, cacheRoot }) {
  ...
  // 7) assets 内容指纹 + Cloudflare Pages 缓存头。
  fingerprintAssets(paths.stagingSiteDir, statusMap);
  writeHeadersFile(paths.stagingSiteDir, statusMap);
