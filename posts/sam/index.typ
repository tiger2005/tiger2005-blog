...
需要注意，中间的状态不仅存在来自红色状态的转移边，还存在一些来自其他的转移边（图中表示为灰色状态的转移边）。算法的第一步是添加一个新的状态 $C$ 来表示结束位置集合 ${n + 1}$。此时，$"len"(C) = n + 1$，而 $"minlen"(C)$ 是未知的，因此我们暂时无法确认 $C$ 的父状态。
...
#note(title: "后缀自动机的构建算法是线性的")[
  ...
  *证明：*在上述代码中，每次扩展一个字符，使用势能分析可以确认除了 29 至 32 行的循环之外，其他部分的时间复杂度为均摊 $cal(O) (1)$。因此，算法的时间复杂度主要取决于 29 至 32 行的循环。
  ...
  这里可以使用 $E(P) = "len"("link"("link"(P)))$ 作为势能函数，不难发现循环在执行不超过两次之后，对应的 $"len"(P')$ 就会小于这个势能函数的值。随后考虑循环的终点，此时 $P'$ 通过 $c$ 可以转移到 $"link"(Q_0) = "link"("link"(C)))$，简单分析可以得到 $"len"(P') = "len"("link"("link"(C))) - 1$。因此，这一次循环的执行次数不超过 $E(P) - E(C) + cal(O)(1)$，而 $C$ 又会作为下一个扩展的 $P$，那么将上限进行累加后就可以得到算法的时间复杂度为 $cal(O) (n)$。上述证明中一些边界情况在此略去。$qed$
]
