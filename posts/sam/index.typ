...
#success(title: "【NOI2018】你的名字")[
  ...
  *解法：*对问题进行容斥，转化为计算 $t$ 的本质不同子串数量，减去同时出现在 $t$ 和 $s_[l, r]$ 中的字符串数量。
  ...
  首先解决 $l = 1, r = |s|$ 的情况。对于字符串 $s$ 构造出后缀自动机，对于询问字符串 $t$，可以计算出其每个前缀在 $s$ 中的最长匹配后缀。随后，对于字符串 $t$ 也构造出后缀自动机，并计算出 $t$ 的本质不同子串数量。对于 $t$ 中长度为 $k$ 的前缀，已知其在 $s$ 中的最长匹配后缀的长度为 $L_k$，考虑在 $t$ 的后缀自动机中添加完前 $k$ 个字符后，结束位置集合 ${k}$ 对应的状态为 $V_k$，则 $V_k$ 及其所有祖先状态对应的字符串，只要长度不超过 $L_k$，就一定同时出现在 $t$ 和 $s$ 中。将长度限制放在 $V_k$ 上，随后遍历 $t$ 的后缀自动机将限制上传，最后计算出同时出现在 $t$ 和 $s$ 中的字符串数量，得到答案。
...
- *充分性：*对于两个字符串 $s_1, s_2$，如果它们是 Myhill-Nerode 不等价的，那么必然存在一个字符串 $t$，使得 $s_1 + t$ 和 $s_2 + t$ 中恰有一个是 $s$ 的后缀。不失一般性的，假设 $s_1 + t$ 是 $s$ 的后缀，而 $s_2 + t$ 不是 $s$ 的后缀。不难发现 $t$ 也是 $s$ 的后缀，并且此时 $s_1$ 是 $s_[1,|s|-|t| ]$ 的一个后缀，而 $s_2$ 不是 $s_[1, |s|-|t| ]$ 的一个后缀。根据结束位置集合的定义，$|s|-|t|$ 是 $"endpos"(s_1)$ 的元素，而不是 $"endpos"(s_2)$ 的元素。
